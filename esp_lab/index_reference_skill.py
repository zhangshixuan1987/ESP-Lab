"""Utilities for external climate index references (e.g. NOAA PSL) and reference-sensitive skill metrics.

Note on historical naming:
--------------------------
This module was formerly named ``psl_skill.py``, where "PSL" referred to NOAA's
Physical Sciences Laboratory (the source of the external benchmark index files
packaged under ``external/``). To avoid confusion with Sea Level Pressure (PSL),
the module has been renamed to ``index_reference_skill.py``.
"""
...
def retain_complete_seasonal_leads(
    ...
    *,
    ...
    dropped_values = [int(value) for value in dropped.values]
    if expected_count is not None and complete.size != expected_count:
        prefix = f"{label}: " if label else ""
        raise ValueError(
            f"{prefix}expected {expected_count} complete centered seasons, "
            f"but found {complete.size} at leads {[int(v) for v in complete.values]}"
        )
...
INDEX_REFERENCE_FILES: Dict[str, str] = PSL_INDEX_FILES
load_index_reference = load_psl_index
index_reference = psl_reference
