"""Versioned cache contracts for lead-time skill workflows and prepared inputs.

Note on historical naming:
--------------------------
This module was formerly named ``prepared_skill.py``. Because it prepares and
caches analysis-ready hindcast anomalies and climatologies (inputs for skill
evaluation) rather than calculating the skill metrics themselves, it has been
renamed to ``leadtime_prepared_cache.py`` (consistent with ``leadtime_skill_cache.py``).
"""
