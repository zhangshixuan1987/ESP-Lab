...
def drop_feb29(ds, time_name="time"):
...
def time_set_midmonth(ds, time_name):
    ...
    values (day=15) for the represented model-output month.

    Model monthly means are end-of-period data. If a timestamp is on day 1,
    treat it as the beginning of the following month and shift it back one
    month. Otherwise, keep the timestamp month and set day 15.
    ...
    shift_back = day == 1
    year = xr.where(shift_back & (month == 1), year - 1, year)
    month = xr.where(shift_back, xr.where(month == 1, 12, month - 1), month)

    nmonths = len(month)
    newtime = []
    for i in range(nmonths):
        template = ds[time_name].values[i]
        yy = int(year[i])
        mm = int(month[i])
        if isinstance(template, (np.datetime64, pd.Timestamp)):
            newtime.append(pd.Timestamp(year=yy, month=mm, day=15))
        elif hasattr(template, "year") and hasattr(template, "month"):
            try:
                newtime.append(type(template)(yy, mm, 15))
            except TypeError:
                newtime.append(cftime.DatetimeNoLeap(yy, mm, 15))
        else:
            newtime.append(cftime.DatetimeNoLeap(yy, mm, 15))
