...
--------------------------
This module was formerly named ``land_prepared_skill.py``. Because it stages
land hindcasts and observational reference products (creating analysis-ready inputs)
rather than calculating skill metrics, it has been renamed to ``land_input_cache.py``.
"""
...
_LAND_PREPARED_CACHE_SCHEMA_VERSION = "2"
...
def expected_attrs(
    *,
    ...
    field = field.upper()
    if not str(regridding_method).strip():
        raise ValueError("regridding_method must be a non-empty string")
    if not isinstance(regridding_periodic, bool):
        raise TypeError("regridding_periodic must be boolean")
    ...
    attrs: dict[str, str | int | float] = {
        ...
        "regridding_periodic": str(bool(regridding_periodic)).lower(),
...
def validate_dataset(
    ...
    *,
    ...
    from .leadtime_prepared_cache import validate_cache_attrs

    validation_expected = dict(expected)
    legacy_periodic = (
        validation_expected.get("regridding_periodic") == "true"
        and "regridding_periodic" not in dataset.attrs
        and dataset.attrs.get("land_prepared_cache_version") == "2"
    )
    if legacy_periodic:
        # Cache schema 2 originally hard-coded periodic=True during both model
        # and reference regridding but did not record it. That omission is
        # therefore safe to interpret as true; false still requires a rebuild.
        validation_expected.pop("regridding_periodic")
    validate_cache_attrs(dataset, validation_expected, required_variables=required)
    da = dataset[field]
...
def prepared_cache_status(path, expected, *, grid: xr.Dataset) -> tuple[bool, str]:
    ...
    try:
        ...
        return (ok, reason) if not ok else (False, str(exc))
    if not ok:
        return True, "compatible legacy cache (periodic regridding was implicitly true)"
    ...
    try:
        if is_target_grid(native, grid):
            ...
            regridder = regrid.make_regridder(
                ...
                method=str(expected["regridding_method"]), periodic=periodic,
            )
...
def prepare_model_cache(
    *,
    ...
    periodic: bool = True,
    write_options: Mapping | None = None,
    resolved_files: Sequence[str | Path] | None = None,
) -> Path:
    """Prepare, validate, and atomically write one E3SM case/month cache."""
    init_tags = data_access_e3sm.build_init_tags(years, init_month)
    resolved_groups = None
    if resolved_files:
        expected_count = len(init_tags) * len(members)
        if len(resolved_files) != expected_count:
            raise ValueError(
                f"resolved_files contains {len(resolved_files)} paths; "
                f"expected {expected_count} for {len(init_tags)} initializations "
                f"and {len(members)} members"
            )
        resolved_groups = [
            list(map(str, resolved_files[offset:offset + len(members)]))
            for offset in range(0, expected_count, len(members))
        ]
    monthly = land_skill.load_e3sm_land_monthly(
        ...
        resolved_files=resolved_groups,
    )
    try:
        selection = ({
        ...
        seasonal = land_skill.seasonal_land_hindcast_dataset(
            monthly, field,
            validate_finite_leads=strict_member_completeness,
            **selection,
        )
