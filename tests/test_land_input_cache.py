...
from esp_lab import land_input_cache
...
def _expected(**overrides):
    ...
    return land_input_cache.expected_attrs(**values)
...
def test_expected_attrs_change_with_requested_years_members_and_source():
    ...
    assert baseline != _expected(regridding_periodic=False)
    assert baseline["initialization_month"] == 5
    assert baseline["regridding_periodic"] == "true"

    with pytest.raises(TypeError, match="must be boolean"):
        _expected(regridding_periodic="yes")


def test_prepared_cache_status_validates_metadata_shape_and_grid(tmp_path):
    grid = land_input_cache.target_grid(90, 180)
    expected = _expected()
    path = tmp_path / "prepared.nc"
    _model_dataset(expected, grid).to_netcdf(path)

    assert land_input_cache.prepared_cache_status(
        path, expected, grid=grid
    ) == (True, "compatible")

    changed = dict(expected, monthly_lead_count=12)
    ok, reason = land_input_cache.prepared_cache_status(path, changed, grid=grid)
    assert not ok
    assert "monthly_lead_count" in reason


def test_schema_two_cache_without_periodic_attr_is_accepted_as_true(tmp_path):
    grid = land_input_cache.target_grid(90, 180)
    expected = _expected(regridding_periodic=True)
    dataset = _model_dataset(expected, grid)
    dataset.attrs.pop("regridding_periodic")
    path = tmp_path / "legacy-periodic.nc"
    dataset.to_netcdf(path)

    ok, reason = land_input_cache.prepared_cache_status(path, expected, grid=grid)

    assert ok
    assert "legacy cache" in reason


def test_schema_two_cache_without_periodic_attr_is_rejected_for_false(tmp_path):
    grid = land_input_cache.target_grid(90, 180)
    expected = _expected(regridding_periodic=False)
    dataset = _model_dataset(expected, grid)
    dataset.attrs.pop("regridding_periodic")
    path = tmp_path / "legacy-nonperiodic.nc"
    dataset.to_netcdf(path)

    ok, reason = land_input_cache.prepared_cache_status(path, expected, grid=grid)

    assert not ok
    assert "regridding_periodic" in reason
...
def test_reference_file_resolution_is_stable_and_nonempty(tmp_path):
    ...
    assert land_input_cache.resolve_reference_files(str(tmp_path / "*.nc")) == [
        first, second
    ]
    with pytest.raises(FileNotFoundError):
        land_input_cache.resolve_reference_files(str(tmp_path / "missing*.nc"))


def test_revision_identity_does_not_require_an_inventory():
    identity = land_input_cache.raw_source_identity(
...
def test_inventory_snapshot_restart_preserves_exact_land_identity(tmp_path):
    ...
    inventory = land_input_cache.raw_source_identity(
        paths=[source], mode="inventory", **options
    )
    source.unlink()
    assert land_input_cache.raw_source_identity(
    ...
    with pytest.raises(RuntimeError, match="run once in inventory mode"):
        land_input_cache.raw_source_identity(paths=[], mode="snapshot", **changed)
...
def test_snapshot_rejects_tampered_record(tmp_path):
    ...
    land_input_cache.raw_source_identity(mode="inventory", **options)
    record = next((tmp_path / "snapshots").glob("*.json"))
    record.write_text("{}")
    with pytest.raises(ValueError, match="Invalid land source"):
        land_input_cache.raw_source_identity(mode="snapshot", **options)


def test_prepared_data_identity_changes_with_contract():
    assert land_input_cache.prepared_data_identity(_expected()) != (
        land_input_cache.prepared_data_identity(
