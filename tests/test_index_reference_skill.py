...
def test_load_psl_index_reads_packaged_csv(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(index_reference_skill.PSL_INDEX_FILES, "IOD", "iod.HadISST.PSL.csv")

    da = index_reference_skill.load_psl_index("IOD", external_dir=tmp_path)
...
def test_seasonal_centered_mean_keeps_season_center_months():
    ...
    out = index_reference_skill.seasonal_centered_mean(da)
...
def test_seasonal_centered_mean_rejects_all_missing_input():
    ...
    with pytest.raises(ValueError, match="contains no finite values"):
        index_reference_skill.seasonal_centered_mean(da)
...
def test_retain_complete_seasonal_leads_drops_only_all_missing_endpoint():
    ...
    kept, kept_time, dropped = index_reference_skill.retain_complete_seasonal_leads(data, valid_time)
...
def test_retain_complete_seasonal_leads_rejects_mismatched_coordinates():
    ...
    with pytest.raises(ValueError, match="identical L coordinates"):
        index_reference_skill.retain_complete_seasonal_leads(data, valid_time)


def test_retain_complete_seasonal_leads_validates_expected_count():
    data = xr.DataArray([1.0, np.nan], dims="L", coords={"L": [3, 6]})
    valid_time = xr.DataArray([0, 1], dims="L", coords={"L": [3, 6]})

    # Passes when expected_count matches kept count (1)
    kept, _, dropped = index_reference_skill.retain_complete_seasonal_leads(
        data, valid_time, expected_count=1, label="TestCase"
    )
    assert len(kept.L) == 1

    # Raises when expected_count differs
    with pytest.raises(ValueError, match="TestCase: expected 2 complete centered seasons, but found 1"):
        index_reference_skill.retain_complete_seasonal_leads(
            data, valid_time, expected_count=2, label="TestCase"
        )
...
def test_subset_hindcast_initialization_years_preserves_later_valid_times():
    ...
    selected, selected_time = index_reference_skill.subset_hindcast_initialization_years(
...
def test_subset_hindcast_initialization_years_rejects_incomplete_cohort():
    ...
    with pytest.raises(ValueError, match="Expected every initialization year"):
        index_reference_skill.subset_hindcast_initialization_years(data, valid_time, 2000, 2002)
...
def test_observation_agreement_removes_local_monthly_climatology():
    ...
    result = index_reference_skill.observation_agreement(
        local_raw,
        psl_anom,
        2000,
        2003,
...
def test_add_reference_sensitivity_adds_deltas_and_robust_metrics():
    ...
    out = index_reference_skill.add_reference_sensitivity(skill)
...
def test_compute_reference_skill_accepts_raw_and_anomaly_references():
    ...
    skill = index_reference_skill.compute_reference_skill(
        model,
        model_time,
        {
...
def test_aliases_match():
    from esp_lab import index_reference_skill
    assert index_reference_skill.INDEX_REFERENCE_FILES is index_reference_skill.PSL_INDEX_FILES
    assert index_reference_skill.load_index_reference is index_reference_skill.load_psl_index
    assert index_reference_skill.index_reference is index_reference_skill.psl_reference
