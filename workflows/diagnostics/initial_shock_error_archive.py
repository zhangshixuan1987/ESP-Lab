...
ARCHIVE_VERSION = "initial_shock_rmse_mae_archive_v1"
...
def _cache_valid(path, digest):
    ...
    try:
        with xr.open_dataset(path) as ds:
            required = {"rmse", "mae", "model_anomaly", "observation_anomaly",
...
def plan_archive_run(settings, cases, variable):
    ...
    metric_code = Path(compute_initial_shock_error_index.__code__.co_filename)
    metric_hash = hashlib.sha256(metric_code.read_bytes()).hexdigest()
    for task in plan:
        identity = {
        ...
        task["error_rebuild"] = settings["cache"]["mode"] == "rebuild" or not task["error_cached"]
...
def compute_archive_plan(plan, settings, variable):
    ...
    for month, comparison in block_by_month.items():
        ...
        for case in comparison.case.values:
            ...
            if settings["cache"]["mode"] != "rebuild" and _cache_valid(path, task["error_digest"]):
